...
func main() {
	...
	filepath.Walk("examples/step1-code/html", func(path string, fi os.FileInfo, err error) error {
	...
	// gographviz.Edge.
	codeArs := make(map[string]*AggregateRoot)
					...
					if edge.Attrs["style"] == "\"dashed\"" {
						...
						if nodes[edge.Dst] != "AggregateRoot" {
							if nodes[edge.Src] == "AggregateRoot" {
								codeArs[nodes[edge.Dst]] = &AggregateRoot{name: nodes[edge.Dst]}
							}
